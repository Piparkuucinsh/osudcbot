...
import { MapPlayCount, Demographic } from '@/models'
...
const popular_maps: CommandModule = {
    data: new SlashCommandBuilder()
        ...
        .setDescription(
            'View the most popular maps in the server, filter by users or roles'
        )
        .addIntegerOption((option) =>
            option
                .setName('limit')
                .setDescription(
                    'The number of maps to show in the leaderboard - default 10'
                )
                .setRequired(false)
        )
    ...
    execute: async (interaction: CommandInteraction) => {
        ...
        try {
            ...
            const { users, replied, raw_collect } = await collectUsers(
                filter_by,
                interaction
            )
            if (replied) {
...
async function getPopularMaps(
    users: string[],
    limit: number
): Promise<MapPlayCount[]> {
    const mapIds = new Set<bigint>()
    for (const user_id of users) {
        try {
            ...
            if (!osu_user_id) {
                throw new Error(
                    `Osu user not found in database for discord user: ${user_id}`
                )
            }
            const activities = await v2.user.activity(parseInt(osu_user_id), {
                limit: 10,
            })
            console.log(activities)
            activities.forEach((activity) => {
                if (activity.type === 'rank') {
                    const beatmapId = BigInt(
                        parseInt(activity.beatmap.url.split('/')[2])
                    )
    ...
    const mapPlayCounts: MapPlayCount[] = []
    for (const mapId of mapIds) {
        // Fetch play count for each map
        const details = await v2.beatmap.id.details(parseInt(mapId.toString()))
        mapPlayCounts.push({
            beatmapId: mapId,
            beatmapName: details.beatmapset.title,
            playCount: details.beatmapset.play_count,
            beatmapCover: details.beatmapset.covers.cover,
        })
    }
...
async function createMapPoll(
    ...
    const description = mapOptions
        .map((map, index) => `${alphabet[index]}: ${map.beatmapName}`)
        .join('\n')
