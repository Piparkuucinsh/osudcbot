...
function getUsername(interaction: CommandInteraction): [string, string] {
    const usernameOption1 = interaction.options.data.find(
        (opt) => opt.name === 'username_1'
    )
    const usernameOption2 = interaction.options.data.find(
        (opt) => opt.name === 'username_2'
    )
...
function compareAndEmbedStatistics(
    ...
    const keys: (keyof UserStatistics)[] = [
        'pp_score',
        'accuracy',
        'play_count',
        'total_score',
